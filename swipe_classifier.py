...
class SwipeClassifier:
    ...
    def classify_swipe(self, frame, debug_img=False):
        ...
        shoulder_width, nose = self._get_normalization_factors(
            keypoints)
        self.shoulder_width = shoulder_width
        self._move_thresh = shoulder_width*.5
        self._elbow_circle_raidus = self.shoulder_width*3.5
        self._nose_seq.append(nose)

        if self._person_valid(keypoints, self._nose_seq):
            ...
            frame = self._debug_draw(frame, keypoints) if debug_img else frame

        return out, frame if debug_img else out

    def _debug_draw(self, img, kps) -> np.array:
        frame = Movenet.draw_keypoints(img, kps)
        return frame

    def _update_state(self, states, position, positions_seq) -> Optional[Position]:
        if not states:
            states.append(position)
        elif states and not states[-1] == position:
            states.append(position)
            return self._detect_swipe(states, positions_seq)
    ...
    def _person_valid(self, keypoints_with_scores, seq) -> bool:
        if len(seq) == 0:
            return True
        # is person moving
        if self._move_displacement(seq)[1] <= self._move_thresh and .35 >= self.shoulder_width >= .12:
            return True
        return False

    @staticmethod
    def _get_normalization_factors(kps) -> tuple:
    ...
    @staticmethod
    def _normalize_wrists(kps, scl_factor) -> tuple:
        """
